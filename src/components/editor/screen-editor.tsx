...
export function ScreenEditor({ mockupId, initialConfig, userRole = "user" }: ScreenEditorProps) {
  ...
  const readFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      setWindows(prev => prev.map((w, i) =>
        i === activeWin ? { ...w, screenshot: e.target?.result as string } : w
      ));
    };
  ...
  const handleExport = async (scale: number) => {
    ...
    if (!isPro(userRole)) {
      ...
      watermark.style.cssText = `
        position:absolute;bottom:12px;right:14px;z-index:9999;
    ...
    <>
      ...
      <div className="flex h-[calc(100vh-56px)]">
          ...
          <div className="p-4 space-y-5">
            ...
            <section>
              <p className="text-[10px] uppercase tracking-widest text-text-secondary mb-2 font-medium">
                {currentFrame.supportsUrl ? "URL" : "Window Title"} — Win {activeWin + 1}
              </p>
              <Input
            ...
            {/* Screenshot */}
            <section>
              <p className="text-[10px] uppercase tracking-widest text-text-secondary mb-2 font-medium">Screenshot — Win {activeWin + 1}</p>
              <button
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-0.5 bg-surface-elevated rounded-xl border border-white/7 p-0.5">
                {[1, 2, 3].map(s => (
                  <button
                    ...
                    disabled={exporting}
                    title={s === 3 && !pro ? "Pro only" : `Export at ${s}×`}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all disabled:opacity-40 relative ${s === 3 && !pro ? "text-text-secondary/50 cursor-default" : "text-text-secondary hover:text-text-primary hover:bg-white/5"}`}
                  >
                    {s}×
                    {s === 3 && !pro && <Zap size={7} className="absolute -top-0.5 -right-0.5 text-accent"/>}
                  </button>
                ))}
              </div>
          ...
          <div className="flex-1 overflow-auto editor-canvas flex items-center justify-center p-8"
            onDragOver={e => { e.preventDefault(); setDragging(true); }}
            onDragLeave={() => setDragging(false)}
            onDrop={onDrop}
          >
            <div
              ref={canvasRef}
              style={{
                background: wallpaper.bg, borderRadius: 12, overflow: "hidden",
                position: "relative", width: "min(900px,100%)", aspectRatio: "16/10",
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: `${16 + back * 18}px ${24 + back * 18}px`,
                        zIndex: isActive ? windows.length : i,
                        transition: "all 0.2s ease",
                        cursor: isActive ? "default" : "pointer",
                        filter: isActive ? "none" : `brightness(0.55) blur(0.5px)`,
                        transform: isActive ? "none" : `scale(${0.96 - back * 0.02}) translateY(${back * 6}px)`,
                      }}
                    >
